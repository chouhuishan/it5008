...
/**
  ...
  * and the "faculty" table on the department afterwards.
  */
SELECT name, faculty
FROM student, copy, department
WHERE student.email = copy.owner
AND student.department = department.department; -- Step 1a
...
/**
  ...
  * "c" for "copy", and "l" for "loan", etc. (refer Step 1b).
  */
SELECT s.name, d.faculty
FROM student s, copy c, department d
WHERE s.email = c.owner
AND s.department = d.department; -- Step 1b
...
/**
  ...
  */
SELECT s.name, d.faculty
FROM student s, loan l, department d
WHERE s.email = l.borrower
  AND l.returned ISNULL
  AND s.department = d.department;  -- Step 1c
...
/**
  ...
  * To differentiate them, let's modify the table aliases to s1 and d1 for the owner,
  * s2 and d2 for the borrower.
...
FROM student s1, student s2,
  ...
  copy c, loan l          -- the order of these do not matter
